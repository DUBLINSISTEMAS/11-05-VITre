...
- **Próxima fase**: **Fase 2 — pagamento configurável** (ADR-0013 a ser escrito). Resolve bug atual real (`formatInstallments` hardcoded em `src/lib/pricing.ts:80` usando preço promocional). Adiciona colunas em `storeTable` e `productTable`, refactor de `pricing.ts`, nova seção em `/admin/configuracoes`.
...
- **Dívidas conhecidas da auditoria 2026-05-12** (5 críticos abertos): dialog duplicado em `/admin/produtos`, cache de `promoOnly` vazando até 5min em `unstable_cache`, `useEffect` com dep array de objeto em `ProductDialog`, `withTenant("")` que quebra futura policy UUID, `key={NEW_PRODUCT_ID}` constante em save-and-continue. Detalhes em `docs/sessoes/2026-05-12-auditoria-senior-pendencias.md`. Fechar antes de Fase 5 (PDV), porque PDV escreve em produto/pedido com padrões parecidos.
