...
- **Bloco 5b** ✅ INFRA — SEO canonical + redirect opt-in via 2 env vars. Helper `lib/storefront/canonical-url.ts` centraliza; `generateMetadata` (PDP, categoria, coleção) + sitemap.ts usam. Default mantém path. Quando founder flipar `STOREFRONT_CANONICAL_HOST_STYLE=subdomain` (após wildcard pronto), Google reindexa subdomain como canônico. Em seguida `STOREFRONT_REDIRECT_TO_SUBDOMAIN=true` consolida tráfego via 301. Fix de gap real: PDP NÃO tinha canonical antes (Google podia indexar variantes `?utm_*`).
