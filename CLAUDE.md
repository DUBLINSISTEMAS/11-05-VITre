...
- **Port Dublin v3 (BAGY-style) CONCLUÍDO em 2026-05-17** ([ADR-0019](docs/decisoes/0019-port-dublin-v3-bagy-style.md)). 13 commits em prod (`6079bec..8fbee55`): Ondas 0-1 (ADR + tokens), Onda 2 (Login/Redefinir/Recuperar split-brand), Onda 3 (Onboarding 4 passos com tipo-negócio), Onda 4 (Admin shell sidebar 248px navy), Ondas 5a-5h (refresh módulos: Dashboard → Produtos → Categorias+Banners → Pedidos → Clientes → Estoque → PDV → Aparência+Configurações+Pagamento). **Onda 6 (Storefront) = SKIP confirmado por auditoria** — `PIXEL PERFECT/storefront/vitre.jsx:4` autodeclara "Recreated from src/components/storefront/*", ou seja o pacote foi extraído do código atual, não o inverso; storefront atual é superset do alvo (BrandProvider scoping, variants por preset, drawers reais, ISR/favoritos), mobile-frame fake 390×844 vetado pelo ADR-0019. Brand admin mudou **carvão canvas-v1 → navy BAGY** `#1A3A8F`; `--brand-store` por loja preservado (ADR-0011). Pausa "Onda 2 hotfix → relatórios" do roadmap gestão completo permanece. Módulos novos sem backend (atributos, cupons, grupos, equipe, assinatura, suporte) **ficam fora do port** — backlog pós.
