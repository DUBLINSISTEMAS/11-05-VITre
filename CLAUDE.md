...
- **Port Dublin v3 (BAGY-style) REABERTO em 2026-05-17 tarde** pra **HIGH-FIDELITY pixel-perfect total** ([ADR-0019](docs/decisoes/0019-port-dublin-v3-bagy-style.md) será revisado). Ondas 0-5i fechadas em prod (`6079bec..e918e57`) entregaram ~10-15% do alvo — fizeram **token sweep + sidebar 248px + login split + onboarding 4-passos**, NÃO o pixel-perfect HIGH-FIDELITY (cores exatas, tipografia, espaçamentos, componentes finalizados) que o pacote oficial Dublin v3 pedia. Founder reabriu escopo pra Caminho A: **44 telas pixel-perfect + módulos novos com backend novo** (Atributos / Cupons / Grupos cliente / Equipe multi-user 3 roles / Assinatura / Suporte). Override consciente do framing `vitre-substitui-excel-whatsapp-nao-bling`. Brand admin **carvão → navy** `#1A3A8F`; `--brand-store` por loja preservado (ADR-0011). Tokens canônicos hoje vivem direto em `src/app/globals.css` + `tailwind.config.ts` (pasta `PIXEL PERFECT/` removida 2026-05-19 — handoff já cumpriu o papel, código é a SoT). Próximas ondas exigem ADRs novos (ADR-0020 Atributos / 0021 Cupons / 0022 Grupos cliente / 0023 Equipe que **quebra invariante "Better Auth somente lojista"**).
...
- **Port Dublin v3 (SoT visual ATIVA)**: [`docs/decisoes/0019-port-dublin-v3-bagy-style.md`](docs/decisoes/0019-port-dublin-v3-bagy-style.md). Tokens canônicos hoje em `src/app/globals.css` + `tailwind.config.ts` (pasta `PIXEL PERFECT/` removida 2026-05-19 após cumprir papel de handoff).
