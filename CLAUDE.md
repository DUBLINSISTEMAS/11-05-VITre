...
- **Próxima fase**: **Fase 4 — estoque event-sourced** (ADR-0015 proposto 2026-05-16). Tabela `stock_movement` como fonte de verdade, `stock_quantity` em produto/variante vira cache denormalizado atualizado via trigger SQL. Refactor de `create-from-cart.ts` (sale movement em vez de UPDATE direto), `update-status.ts` (return automático em cancelamento), form de produto (botão "lançar movimentação"). Nova rota `/admin/estoque` com relatório de movimentações. Backfill 1 `initial` movement por produto/variant com saldo > 0.
