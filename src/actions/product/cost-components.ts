...
export async function saveCostComponents(input: z.infer<typeof SaveSchema>) {
  ...
  await withTenant(store.id, session.user.id, async (tx) => {
    const [prod] = await tx
      ...
      .from(productTable)
      .where(
        and(eq(productTable.id, productId), eq(productTable.storeId, store.id)),
      )
      .limit(1);
    if (!prod) throw new Error("Produto não encontrado");

    await tx
      .delete(productCostComponentTable)
      .where(
        and(
          eq(productCostComponentTable.productId, productId),
          eq(productCostComponentTable.storeId, store.id),
        ),
      );

    if (components.length > 0) {
      await tx
        .insert(productCostComponentTable)
        .values(
          components.map((c, i) => ({
            storeId: store.id,
            productId,
    ...
    await tx
      .update(productTable)
      .set({ costPriceInCents: totalInCents })
      .where(
        and(eq(productTable.id, productId), eq(productTable.storeId, store.id)),
      );
  });
