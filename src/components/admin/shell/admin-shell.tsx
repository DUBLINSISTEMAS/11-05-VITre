 ...
 *
 * Layout:
 * - Desktop (lg+): b3-shell flex → b3-side (sidebar) + b3-main →
 *   `.b3-main-card` (card branco flutuante) com `TopBar` (chrome interno:
 *   busca ghost + sino + ?) no topo do card e conteúdo abaixo. Refactor
 *   2026-05-29: antes a TopBar flutuava acima do card.
 * - Mobile (<lg): MobileHeader sticky (hamburger + logo + sino) + card
 *   full-bleed. Sidebar entra via Sheet drawer no hamburger.
 *
  ...
  return (
    <div className="b3-shell print:block">
      ...
      <div className="b3-main">
        ...
        {/*
          Card branco flutuante (ref Abacate Pay 2026-05-21, refactor
          2026-05-29) — fundo branco com border-radius 20px e margem visivel
          nos 4 lados mostrando o cinza do .b3-main. Scroll do conteudo e
          interno a este card. Em desktop, a `TopBar` agora fica DENTRO do
          card (chrome no topo) — antes flutuava acima dele.
        */}
        <main className="b3-main-card flex-1 px-4 pt-3 pb-12 sm:px-6 sm:pt-3 lg:px-8 lg:pt-0 lg:pb-12 print:px-0 print:py-0 print:m-0 print:rounded-none print:shadow-none">
          <TopBar />
          {/*
          ...
          */}
          <div className="mx-auto w-full max-w-7xl space-y-4 pt-3 sm:space-y-5 sm:pt-4 lg:pt-6 print:max-w-none print:space-y-0 print:pt-0">
            {children}
          </div>
        </main>
      </div>
