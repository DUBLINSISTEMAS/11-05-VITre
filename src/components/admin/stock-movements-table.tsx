...
const TYPE_BADGE_CLASS: Record<StockMovementRow["movementType"], string> = {
  initial: "bg-bg-app text-ink-4",
  manual_in: "bg-ok-wash text-ok",
  manual_out: "bg-danger-wash text-danger",
  sale: "bg-danger-wash text-danger",
  return: "bg-ok-wash text-ok",
  adjustment: "bg-warn-wash text-warn",
};
  ...
  return (
    <>
      ...
      <div className="b3-card hidden overflow-hidden lg:block">
        <div
          role="rowgroup"
          className="text-eyebrow bg-bg-app grid grid-cols-[minmax(0,1.4fr)_minmax(0,140px)_minmax(0,100px)_minmax(0,1fr)_minmax(0,130px)] items-center gap-4 border-b border-line px-4 py-2.5"
        >
          <span>Produto</span>
          <span>Tipo</span>
          ...
          {movements.map((m) => (
            ...
            >
              <Link
                ...
                prefetch={false}
                className="min-w-0 truncate font-medium text-ink-1 hocus:underline"
              >
                {m.productName}
                {m.variantName ? (
                  <span className="text-ink-4"> · {m.variantName}</span>
                ) : null}
              </Link>
              <span>
                <span
        ...
        {movements.map((m) => (
          <li key={m.id} className="px-3 py-2.5">
            <div className="flex items-start gap-2.5">
              <div className="bg-bg-app flex size-9 shrink-0 items-center justify-center rounded-md">
              ...
              <div className="min-w-0 flex-1">
                <Link
                  href={`/admin/produtos/${m.productId}`}
                  prefetch={false}
                  className="block truncate text-[13.5px] font-medium leading-tight text-ink-1 hocus:underline"
                >
                  {m.productName}
                </Link>
                {m.variantName ? (
                  <p className="text-ink-4 text-[11.5px] leading-tight">
                    {m.variantName}
                  </p>
                ) : null}
                ...
                {m.notes ? (
                  <p className="text-ink-4 mt-1 truncate text-[11.5px] leading-snug">
                    {m.notes}
                  </p>
                ) : null}
              </div>
...
function ReferenceCell({
  ...
  if (referenceType === "order" && referenceId) {
    return (
      <span className="text-ink-4 min-w-0 text-[12px]">
  ...
  return (
    <span className="text-ink-4 min-w-0 truncate text-[12px]">
      {notes ?? "—"}
    </span>
  );
}
