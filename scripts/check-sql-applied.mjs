...
const checks = [
  ...
  { id: "73b", desc: "store quota CHECKs positivos", q: "SELECT 1 FROM pg_constraint WHERE conname IN ('store_max_products_count_range','store_max_image_mb_range') HAVING count(*) = 2" },
  ...
  { id: "76a", desc: "store fees columns (debit/credit_1x/2_6x/7_12x) (S2.4 taxa cartão real)", q: "SELECT 1 FROM information_schema.columns WHERE table_name='store' AND column_name IN ('card_real_fee_bps_debit','card_real_fee_bps_credit_1x','card_real_fee_bps_credit_2x_to_6x','card_real_fee_bps_credit_7x_to_12x') HAVING count(*) = 4" },
  { id: "76b", desc: "store card fee CHECKs range 0..9999", q: "SELECT 1 FROM pg_constraint WHERE conname='store_card_real_fee_ranges'" },
